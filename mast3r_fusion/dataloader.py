...
class MonocularDataset(torch.utils.data.Dataset):
    ...
    def subsample(self, subsample, start_from = 0, end_at = -1):
        self.rgb_files = self.rgb_files[start_from:end_at:subsample]
        self.timestamps = self.timestamps[start_from:end_at:subsample]
...
class StampedFiles(MonocularDataset):
    def __init__(self, dataset_path, stamp_path):
        super().__init__()
        self.use_calibration = False
        self.dataset_path = pathlib.Path(dataset_path)
        ...
        if len(data.shape) == 1:
            ...
            dd = data        
        if len(dd.shape) == 1 or dd.shape[1] == 1:
            # self.rgb_files = natsorted(list((self.dataset_path).glob("*.png")))
            self.rgb_files = natsorted(list((self.dataset_path).glob("*.jpg")))
            if len(self.rgb_files) == 0:
                self.rgb_files = natsorted(list((self.dataset_path).glob("*.png")))
            ...
            for i in range(dd.shape[0]):
                self.rgb_files.append(self.dataset_path / dd[i,1])
...
class Intrinsics:
    ...
    @staticmethod
    def from_calib(img_size, W, H, calib, always_undistort=False, model = 'pinhole',scale=1,H_new = None):
        if not config["use_calib"] and not always_undistort:
            return None
        if model == 'pinhole' or model is None:
            fx, fy, cx, cy = calib[:4]
            ...
            if not (H_new is None):
                offset = (H - H_new) / 2
                K_opt[1,2] = cy - offset
                mapx, mapy = cv2.omnidir.initUndistortRectifyMap(
                K, D, xi, np.eye(3), K_opt, (W, H_new), cv2.CV_32FC1, cv2.omnidir.RECTIFY_PERSPECTIVE
                )
                return Intrinsics(img_size, W, H_new, K, K_opt, D, mapx, mapy)
            else:
                mapx, mapy = cv2.omnidir.initUndistortRectifyMap(K,D,xi,np.eye(3),K_opt,(W,H),cv2.CV_32FC1, cv2.omnidir.RECTIFY_PERSPECTIVE)
        # K_opt[0,0] = 900
        # K_opt[1,1] = 900
                return Intrinsics(img_size, W, H, K, K_opt, D, mapx, mapy)
        elif model == 'fisheye':
...
def load_dataset(dataset_path,stamp_path = None):
    ...
    return StampedFiles(dataset_path,stamp_path)
